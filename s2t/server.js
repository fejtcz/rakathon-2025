...
const BACKEND_URL = process.env.BACKEND_URL || "http://localhost:3000/api/store-transcription";
...
app.post("/api/transcribe", upload.fields([
  ...
  execFile("python", [scriptPath, savedAudioPath], { encoding: "utf8" }, async (error, stdout, stderr) => {
    ...
    try {
      ...
      formData.append("audio", fs.createReadStream(savedAudioPath)); // Zvukový soubor
      ...
      // Odpověď klientovi
      return res.json({ message: "Přepis a data úspěšně zpracována a odeslána.", translation });
    } catch (err) {
